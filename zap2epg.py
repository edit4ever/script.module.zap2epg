...
def mainRun(userdata):
    ...
    useLang = False
    useHex = 0
    urlheader = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/115.0'}
    ...
    for setting in settingsDict:
        ...
        if setting == 'urlheader':                          #HTTP headers dictionary for web requests
            urlheader = settingsDict[setting]
                        ...
                        try:
                            if not os.path.exists(fileDir) and EPseries not in failList:
                                ...
                                while retry > 0:
                                    ...
                                    try:
                                        URLcontent = urllib.request.Request(url, data=data_encode, headers=urlheader)
        ...
        while count < dayHours:
            ...
            if not os.path.exists(fileDir):
                try:
                    ...
                    req = urllib.request.Request(url, data=None, headers=urlheader)
